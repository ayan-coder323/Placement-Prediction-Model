...
if len(numeric_columns) >= 2:
   ...
   for target_column in numeric_columns:
       ...
       has_missing_values = bool(df_model[target_column].isnull().any())
...
final_result = pd.concat(
   [
       ...
       deletion_indicator.rename("Deletion_Row_Removed").to_frame(),
       df_mean[numeric_columns].add_suffix("_Mean_Imputed").reset_index(drop=True),
       df_median[numeric_columns].add_suffix("_Median_Imputed").reset_index(drop=True),
       df_model[numeric_columns].add_suffix("_Model_Imputed").reset_index(drop=True),
       df_original.isnull().astype(int).add_suffix("_Missing").reset_index(drop=True),
   ],
   axis=1,
)
